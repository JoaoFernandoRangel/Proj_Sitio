...
bool ativar = true;          // indica se vai rodar a transmissao
    //valor maximo de abertura da garra
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  int receivedValue = receivedMessage.toInt();  // Convert the string to an integer
  ...
  handleReceivedValue(receivedValue);
}

void handleReceivedValue(int value) {
  // Do something with the received value
  // For example, control LEDs based on the received value
  if (value == 10) {
    digitalWrite(33, LOW);
  } else if (value == 11) {
